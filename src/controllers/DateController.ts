...
import { IDateEventParam, AlertService } from '@zeedhi/common';
...
export class DateController {
    ...
    public selectDate({ date }: IDateEventParam) {
        this.date = `${date}`;
        console.log(this.date);
    }

    public selectDate1({ date }: IDateEventParam) {
        this.date1 = `${date}`;
        console.log(this.date1);
    }

    public request(cat: string) {
        axios.get(`https://newsapi.org/v2/everything?q=a&from=${this.date}&to=${this.date1}&sortBy=popularity&apiKey=`)
            .then((response) => {
